...
import glob, os
#---------------------------------------------------------------------------------
data = pd.read_csv("AIS_2020_06_01_region_1.csv")
data_all_MMSI_list = data.MMSI.unique().tolist()
#MMSI_array = [316001251, 367542320, 366961250, 308516000, 316001238, 308045000]
MMSI_array = data_all_MMSI_list
total_ship_number = 30
scatter = 0
path = './Real_data/2020_06_01/'
scatter_file = glob.glob(os.path.join(path, "segment_oneship*_2020_06_01_region_1.csv"))
print(scatter_file)

for i in range(1,total_ship_number+1):
    shipnumber = i
    data_processing_1 = 1
    filename_processing_1 = ("./Real_data/2020_06_01/data_oneship_%d_2020_06_01_region_1.csv" %(shipnumber))
    data_processing_2 = 0
    filename_processing_2 = ("./Real_data/2020_06_01/data_oneship_%d_2020_06_01_new_region_1.csv" %(shipnumber))
    draw = 0
    filename_draw = ("./Real_data/2020_06_01/segment_oneship_%d_2020_06_01_region_1.csv" %(shipnumber))
    mapname_draw = ('./Map/2020_06_01/ocean_map_%d_2020_06_01_region_1.html' %(shipnumber))

    MMSI = MMSI_array[shipnumber-1]
    #----------------------------------------------------------------------------------
    if (data_processing_1 == 1):
        path = './'
        file = glob.glob(os.path.join(path, "AIS_2020_06_01_region_1.csv"))
        print(file)
        Data = []
        for f in file:
            data = pd.read_csv(f)
...
#---------------------------------------------------------------------------------------------
    if(draw == 1):
        ...
        for i in range(M, N):
            segment_name = ("./Real_data/2020_06_01/segment_ship_%d_period_%d_2020_06_01_region_1.csv" %(shipnumber,i))
            p = pd.Period('2020-06-01', freq = '10T')
